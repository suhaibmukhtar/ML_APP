...
## Peform the Clustering of the Data
...
#perform the Multi-class classification of Data
...
#calculate duration and number of Pick and place for each day
def task3(data):
    ...
    data2=data.copy()
    data2=data2[['datetime','position','location','total_duration_seconds']]
    ...
    return data2,activity_counts
#App using ML Suhaib mukhtar
app = Flask(__name__)

@app.route('/duration')
def duration():
    data3 = pd.read_excel('rawdata.xlsx')
    df_duration, _ = task3(data3)
    app_name = "Task_3 Duration Results"
    return render_template('duration.html', app_name=app_name, df_duration=df_duration)
